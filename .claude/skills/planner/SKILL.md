---
name: planner
description: Plan-then-delegate workflow for this portfolio repo. Use this skill for ANY code change to the site — new features, edits, refactors, bug fixes, styling, content updates — even small ones, and even if the user doesn't say "plan". The session model (Fable 5 when available) does the planning and review at maximum reasoning effort; implementation is delegated to the cheapest model that can do the job safely via the implementer-security / implementer-hard / implementer agents, with verification by the tester agent. Do NOT write implementation code directly in the main session unless the change is a one-line fix.
---
...
### 0. Process check (unbendable)

- Read `docs/pm/STATUS.md` first — current phase, gate, and next action.
  All rounds run under the gates and rules in `docs/pm/PROCESS.md`; this
  skill operates inside G1 (plan) and G2 (build). The project invariants
  (PROCESS.md §7) bind every plan and every review below.
...
- **M/L rounds don't solo-draft:** G1 plan drafting uses 2–3 independent
  **Fable** drafter agents with distinct angles (visitor/content value;
  design-language & UX fit; production-readiness) per PROCESS.md §3/§8 —
  you synthesize the drafts into the plan, you don't write it alone.
...
| **Hard (H-class)** | `implementer-hard` | Opus (latest) | multi-file changes, `script.js` module additions or refactors, new pages or sections, `journey.html` scroll engine, `FUN/AQMgame.html`, animation/layout systems, theme-token changes in `styles.css`, anything ambiguous or with tricky interactions |
| **Simple (S-class)** | `implementer` | Sonnet (latest) | single-file, low-risk edits: copy/text changes, small CSS tweaks that don't touch `:root` tokens, adding a link or image, `sitemap.xml`/meta updates, comment/doc edits |

Class is re-verified at dispatch time; two escalations on one task
auto-reclassify it one tier up (record in `docs/pm/TASKS.md`).

### 3. Delegate

Dispatch each task with the Agent tool to the chosen agent. Parallel agents
never share a file — serialize or split first; you commit per task (1:1
task↔commit, message names the T-### id). Every delegation prompt must:
...
### 4. Verify (tester) and review (you, max reasoning)

After each implementer returns:

- **ALL testing goes through the `tester` agent (Sonnet)** — dispatch it to
  run the protocol in `docs/pm/TESTING.md` against the change, regardless of
  which model wrote the code. It reports exact counts, never "tests pass".
- Read the diff / changed regions yourself. You are the last line of defense
  — spot-verify the tester's and implementer's claims (trust but verify).
...
3. Add navbar/drawer links, palette entry, sitemap entry — **implementer** (mechanical, spec'd exactly by the planner)
4. Verify — **tester** (serve, 7 surfaces, both themes, sync cross-diff)
5. Review diffs and the tester's counts — **planner**
