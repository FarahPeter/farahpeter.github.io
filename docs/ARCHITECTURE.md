...
| `journey.html` | ~1980 | "Journey" — immersive Apple-style scrollytelling intro. Nine pinned, scroll-scrubbed scenes: hero, statement, route timeline, **photo reel (`jn-s3b`, "Two years, away")**, craft gallery, **photo reel (`jn-s4b`, "Where it actually runs")**, packet flow, **photo reel (`jn-s5b`, "And the rest of it")**, finale. The three reels share one `buildReel()` factory (full-bleed dissolve + Ken Burns) and derive every timing, the counter and the tick strip from the number of `.jn-frame` figures in the markup — adding a photo is a markup-only change. Photos live in `Files/images/journey/web/` at two widths (`-900`/`-1600`), EXIF-stripped. Self-contained: page-scoped `<style>` + inline engine, all `jn-` prefixed; static fallback when JS is off or `prefers-reduced-motion` is set. |
